 ...
 * 
 * 什么是直接左递归？
 * 如果一个产生式的右部以该产生式的左部开头，则称为直接左递归。
 * 例如：A::=Ab|@|b
 * 产生式 A::=Ab 的右部以 'A' 开头，这就是直接左递归。
 * 
 * 参数说明：
 *   LHS: 产生式的左部（非终结符）
 *   RHS: 产生式的右部（包含多个候选式的向量）
 * 
 * 返回值：
 *   返回一个整数向量，包含所有满足左递归条件的候选式在 RHS 中的索引
 *   如果不存在左递归，则返回 {-1}
 * 
 * 示例：
 *   输入：LHS='A', RHS={"b", "AcB", "@", "Ab"}
 *   分析：RHS[1]="AcB" 以 'A' 开头，RHS[3]="Ab" 以 'A' 开头
 *   返回：{1, 3}
 * 
 * 这样 _removeLeftRecursion_Sentence 函数可以根据返回的索引直接处理对应的候选式
 */
 ...
 * 
 * 为什么要消除左递归？
 * 左递归会导致语法分析器陷入无限循环，因为一个产生式可以不断地调用自己。
 * LL(1) 分析法要求文法是非左递归的，所以我们需要消除左递归。
 * 
 * 消除公式（改写规则）：
 * 对于 A -> Aα | β 这种形式的产生式（其中β不以A开头）
 * 改写为：
 *   A  -> βA'
 *   A' -> αA' | ε
 * 其中 A' 是新引入的非终结符，ε 用 @ 表示
 * 
 * 参数说明：
 *   LHS: 产生式的左部
 *   RHS: 产生式的右部（包含多个候选式）
 *   indexs: _isLeftRecursion 返回的索引，标识哪些候选式是左递归的
 * 
 * 示例详解：
 *   输入：LHS='A', RHS={"b", "AcB", "@", "Ab"}, indexs={1, 3}
 *   
 *   步骤1：分离左递归部分和非左递归部分
 *     - 左递归部分（右部以A开头）：RHS[1]="AcB", RHS[3]="Ab"
 *       提取后：RIGHT_PART_OF_LEFT_RECURSION = {"cB", "b"}
 *     - 非左递归部分：RHS[0]="b", RHS[2]="@"
 *       提取后：RIGHT_PART_WITHOUT_LEFT_RECURSION = {"b", "@"}
 *   
 *   步骤2：引入新非终结符 A'
 *     找到一个未使用的大写字母，假设为 G
 *   
 *   步骤3：构造新产生式
 *     - 原非终结符的新右部 = 非左递归部分 + 新非终结符
 *       A  -> bG | G | @G  (当原右部为@时，只加G不加@)
 *       整理得：A -> bG | G
 *     - 新非终结符的右部 = 左递归部分去掉首字母 + 新非终结符 + ε
 *       G -> cBG | bG | @
 *   
 *   输出：
 *     A -> bG | G
 ...
 * 
 * 什么是间接左递归？
 * 间接左递归是指一个非终结符通过多个产生式的链式调用最终回到自身。
 * 例如：
 *   S -> Aa
 *   A -> Sb
 * 这里 S -> Aa -> Sb -> ... 形成循环，导致 S 间接递归到自己
 * 
 * 为什么要使用拓扑排序确定遍历顺序？
 * 拓扑排序可以帮我们确定非终结符之间的依赖关系。
 * 我们需要按照"后向依赖"的顺序处理非终结符，即先处理被依赖的非终结符。
 * 这样在处理一个非终结符时，所有它依赖的非终结符都已经被处理完毕。
 * 
 * 算法思路：
 * 1. 使用拓扑排序确定处理顺序（_getOrder 函数）
 * 2. 对于每个非终结符：
 *    a) 首先检查是否存在直接左递归，如有则消除
 *    b) 然后通过代入（Substitution）消除间接左递归
 *    - 将该非终结符右部中以其他非终结符开头的候选式
 *      替换为那个非终结符的产生式（代入）
 *    - 重复此过程直到不再产生新的左递归
 *    c) 再次检查是否产生新的直接左递归，如有则消除
 * 
 * 处理顺序说明：
 * 例如：Non_Terminal_Symbol = {'S', 'A', 'B'}
 * 拓扑排序后得到：B -> A -> S（反向依赖顺序）
 * 这样当我们处理 A 时，B 已经处理完毕，可以正确代入
 * 
 * 示例（消除间接左递归）：
 *   原文法：
 *     A -> aB | Bb
 *     B -> Ac | d
 *   
 *   步骤1：处理 B（假设 B 没有直接左递归，直接跳过）
 *   
 *   步骤2：处理 A
 *     - 检查 A -> aB | Bb，B 是一个非终结符且已被处理
 *     - 将 B 代入 A -> Bb：得到 A -> Acc | db
 *     - 现在 A -> aB | Acc | db 存在直接左递归（Acc 以 A 开头）
 *     - 消除直接左递归：
 *       A -> aBG | dbG | G
 *       G -> ccG | @
 *   
 ...
 * 
 * 什么是直接回溯？
 * 直接回溯是指一个非终结符的多个候选式具有相同的前缀（公共左因子）。
 * 例如：A -> ab | ac
 * 这两个候选式都以 'a' 开头，这就是直接回溯。
 * 
 * 为什么要消除回溯？
 * 在 LL(1) 分析中，我们需要根据向前看的一个终结符来决定使用哪个候选式。
 * 如果多个候选式有相同前缀，我们无法仅通过一个终结符来确定选择哪个，
 * 这会导致分析器的确定性被破坏。
 * 
 * 消除方法：提取左公因子（Left Factoring）
 * 将 A -> αβ1 | αγ2 改写为：
 *   A -> αA'
 *   A' -> β1 | β2
 * 
 ...
 * 
 * 返回值：
 *   返回一个整数向量，包含所有满足回溯条件的候选式在 RHS 中的索引
 *   如果不存在回溯，则返回 {-1}
 * 
 * 算法说明：
 *   1. 遍历 RHS 中的每个候选式
 *   2. 提取每个候选式的首字符（忽略非终结符和左部自身）
 *   3. 如果发现两个候选式有相同的首字符，则存在回溯
 *   4. 返回所有以该首字符开头的候选式的索引
 * 
 * 示例：
 *   输入：LHS='A', RHS={"b", "BcB", "@", "Bb"}
 *   分析：
 *     - "b" 首字符是 'b'，加入 FIRST_OF_RHS
 *     - "BcB" 首字符是 'B'，是非终结符，跳过
 *     - "@" 首字符是 '@'，跳过（空串）
 *     - "Bb" 首字符是 'B'，是非终结符，跳过
 *   结果：不存在以终结符开头的公共前缀，无回溯，返回 {-1}
 * 
 * 示例2：
 *   输入：LHS='A', RHS={"ab", "ac", "d"}
 *   分析：
 *     - "ab" 首字符是 'a'，加入 FIRST_OF_RHS
 *     - "ac" 首字符是 'a'，已存在于 FIRST_OF_RHS 中，发现回溯！
 *   返回：{0, 1}（"ab" 和 "ac" 的索引）
 */
...
/*
 * 消除单个产生式中的直接回溯（提取左公因子）
 * 
 * 本函数使用"提取左公因子"的方法来消除直接回溯。
 * 
 * 提取左公因子公式：
 * 对于 A -> αβ1 | αβ2 这种形式（α是公共前缀）
 * 改写为：
 *   A -> αA'
 *   A' -> β1 | β2
 * 其中 A' 是新引入的非终结符
 ...
 *   LHS: 产生式的左部
 *   RHS: 产生式的右部（包含多个候选式）
 *   indexs: _isBackTracking 返回的索引，标识哪些候选式有公共左前缀
 * 
 * 示例详解：
 *   输入：LHS='A', RHS={"b", "BcB", "@", "Bb"}, indexs={1, 3}
 *   
 *   说明：indexs={1,3} 表示 RHS[1]="BcB" 和 RHS[3]="Bb" 有公共左前缀（都以B开头）
 *   
 *   步骤1：提取公共左前缀
 *     - 公共左前缀：B
 *     - 提取后：RIGHT_PART_OF_BACKTRACKING = {"cB", "b"}
 *   
 *   步骤2：分离非回溯部分
 *     - 非回溯部分：RHS[0]="b", RHS[2]="@"
 *     - 提取后：RIGHT_PART_WITHOUT_BACKTRACKING = {"b", "@"}
 *   
 *   步骤3：引入新非终结符 A'
 *     找到一个未使用的大写字母，假设为 G
 *   
 *   步骤4：构造新产生式
 *     - 原非终结符的新右部 = 非回溯部分 + 公共前缀 + 新非终结符
 *       A -> b | @ | BG  (即：b | @ | B G)
 *     - 新非终结符的右部 = 去掉公共前缀后的剩余部分
 *       G -> cB | b
 *   
 *   输出：
 *     A -> b | @ | B G
 ...
 * 
 * 注意：
 *   这个函数只处理直接回溯（公共前缀在原始文法中可见）。
 *   对于间接回溯（需要代入后才能发现的回溯），需要先进行代入操作，
 *   然后再调用本函数消除。
 */
 ...
 * 消除文法的直接回溯
 * 
 * 本函数只处理直接回溯，即在单个非终结符的候选式中存在公共左前缀的情况。
 * 
 * 为什么只处理直接回溯？
 * 间接回溯（需要通过代入非终结符后才能发现的回溯）需要先执行代入操作。
 * 这个代入工作由 _getSubstitutedGrammarForCheck 函数完成。
 * 
 * 处理流程：
 * 1. 使用拓扑排序确定非终结符的处理顺序
 * 2. 对每个非终结符：
 *    a) 检查是否存在直接回溯（调用 _isBackTracking）
 *    b) 如有，调用 _eliminateBackTracking_Sentence 消除
 *    c) 使用 goto 循环继续检查，直到当前非终结符没有回溯为止
 * 
 * 注意：
 *   本函数在消除过程中会同时更新 Language 和 TEMP_Language，
 *   以确保后续的非终结符能够看到最新的文法状态。
 * 
 * 间接回溯的处理流程（在 _getLanguage 中调用）：
 *   1. 调用 _getSubstitutedGrammarForCheck 进行代入
 *   2. 检查代入后的文法是否存在回溯
 *   3. 如有回溯，调用本函数消除
 *   4. 重复直到没有间接回溯
 */
 ...
 * 
 * 什么是无用产生式？
 * 无用产生式是指在文法的任何推导过程中都不可能被用到的产生式。
 * 分为两种：
 * 1. 不可终止的产生式（无法推出任何终结符串）
 * 2. 不可达的产生式（从起始符号无法到达）
 * 
 * 为什么要消除无用产生式？
 * - 使文法更加简洁有效
 ...
 * 算法步骤：
 * 
 * Part 1: 消除不可终止的产生式（找到所有"必要"的非终结符）
 *   1. 初始化 USEFUL_NTS 为所有终结符
 *   2. 反复遍历文法：
 *      - 如果一个产生式的右部只包含"必要的"符号（终结符或已确认必要的非终结符）
 *      - 则该产生式的左部也是必要的，加入 USEFUL_NTS
 *   3. 直到 USEFUL_NTS 不再变化
 *   4. 删除不在 USEFUL_NTS 中的非终结符的产生式
 * 
 * Part 2: 消除不可达的产生式（找到所有"可达"的非终结符）
 *   1. 初始化 AVAILABLE_NTS 为起始符号 S
 *   2. 反复遍历：
 *      - 从起始符号出发，沿产生式右部寻找所有非终结符
 *      - 将新发现的非终结符加入 AVAILABLE_NTS
 *   3. 直到 AVAILABLE_NTS 不再变化
 *   4. 删除不在 AVAILABLE_NTS 中的非终结符的产生式
 */
 ...
 * 
 * 什么是 FIRST 集？
 * FIRST(α) 表示可以从 α 推导出的所有句型的首终结符集合。
 * 如果 α 能推出 ε，则 ε 也在 FIRST(α) 中。
 * 
 * FIRST 集的计算规则：
 *   1. 如果 α 是终结符，则 FIRST(α) = {α}
 *   2. 如果 α 是非终结符，且有产生式 α -> a...，则 a ∈ FIRST(α)
 *   3. 如果 α -> ε，则 ε ∈ FIRST(α)
 *   4. 如果 α -> X1X2...Xn，则：
 *        - FIRST(X1) - {ε} ⊆ FIRST(α)
 *        - 如果 ε ∈ FIRST(X1)，则 FIRST(X2) - {ε} ⊆ FIRST(α)
 *        - 依此类推...
 *        - 如果对所有 Xi 都有 ε ∈ FIRST(Xi)，则 ε ∈ FIRST(α)
 * 
 * 示例：
 *   文法：
 *     A -> Bab | aAB
 ...
 *   
 *   计算 FIRST(B)：
 *     - B -> bA，首字符是 'b'，所以 'b' ∈ FIRST(B)
 *     - B -> cb，首字符是 'c'，所以 'c' ∈ FIRST(B)
 *     - B -> @，空串，所以 '@' ∈ FIRST(B)
 *     - FIRST(B) = {'b', 'c', '@'}
 *   
 *   计算 FIRST(A)：
 ...
 * 
 * 算法说明：
 *   本函数使用迭代方法计算 FIRST 集，直到集合不再变化：
 *   1. 初始化：所有终结符的 FIRST 集是它本身
 *   2. 初始化：将所有右部以终结符开头的产生式加入对应的 FIRST 集
 *   3. 迭代：对每个产生式的右部遍历，
 *        如果首字符是非终结符，将其 FIRST 集（除 ε）加入左部的 FIRST 集
 *        如果该非终结符的 FIRST 集包含 ε，继续处理下一个字符
 *        如果处理到最后一个字符且所有字符的 FIRST 集都包含 ε，则加入 ε
 */
 ...
 * 
 * 什么是拓扑排序？
 * 拓扑排序用于得到有向无环图（DAG）中节点的一个线性序列。
 * 该序列满足：对于图中每一条有向边 (u, v)，u 在序列中都出现在 v 的前面。
 * 
 * 在本程序中的应用：
 * 非终结符是图中的节点，产生式 A -> Bα 表示一条从 A 指向 B 的依赖边。
 * 拓扑排序用于确定非终结符的处理顺序，以便正确处理间接左递归等问题。
 * 
 * 为什么要用拓扑排序？
 * 在消除间接左递归时，我们需要按照"依赖顺序"处理非终结符。
 * 如果 A 的产生式右部以 B 开头（A -> B...），则 A 依赖于 B。
 * 我们需要先处理 B，再处理 A，这样才能正确代入。
 * 
 * Kahn's Algorithm 算法步骤：
 *   1. 计算所有节点的入度（有多少条边指向该节点）
 *   2. 将所有入度为 0 的节点放入队列（这些节点没有依赖）
 *   3. 当队列不为空时：
 *        - 取出队首节点 u，将其加入排序结果
 *        - 遍历 u 的所有出边 (u -> v)，将 v 的入度减 1
 *        - 如果 v 的入度变为 0，将 v 入队
 *   4. 重复步骤 3 直到队列为空
 * 
 * 如何检测环路？
 * 如果排序结果中的节点数小于图中的节点总数，说明图中存在环路（即左递归）。
 * 此时返回一个空向量，表示拓扑排序失败。
 * 
 * 返回值：
 *   成功：返回拓扑排序后的节点序列（反向依赖顺序）
 *   失败（存在环路）：返回一个空向量
 * 
 * 示例：
 *   文法：
 ...
 *   拓扑排序过程：
 *     - 初始：B 的入度为 0，加入队列
 *     - 处理 B：输出 B，A 的入度变为 0，加入队列
 *     - 处理 A：输出 A，S 的入度变为 0，加入队列
 *     - 处理 S：输出 S
 *   
 *   结果：[B, A, S]（反向），即先处理 B，再处理 A，最后处理 S
 */
